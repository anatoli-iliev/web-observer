...
import { openclawStateDir, type Env } from "../paths.js";
...
function readOpenclawConfig(env: Env): Record<string, unknown> | null {
  try {
...
export function skillEntry(slug: string, env: Env): SkillEntry | null {
  ...
  const entry = (entries as Record<string, unknown>)[slug];
  if (typeof entry !== "object" || entry === null || Array.isArray(entry)) return null;
  return entry as SkillEntry;
}

/** Whether a skill is explicitly disabled in the config. */
export function skillDisabled(slug: string, env: Env): boolean {
...
function resolveValue(
  ...
  if (typeof raw === "string") {
    ...
    if (match) {
      ...
      if (value === undefined || value === "") {
        return {
          problem:
            `skills.entries.${slug}.env.${name} is "\${${variable}}", but ${variable} is not ` +
  ...
  if (typeof raw === "object" && raw !== null) {
    ...
    return {
      problem:
        `skills.entries.${slug}.${name} is a secret reference, which only the Gateway can ` +
 ...
 */
export function collectCredentials(spec: CredentialSpec, env: Env): CredentialResult {
  const result: CredentialResult = { env: {}, problems: [], sources: {} };
  ...
  for (const name of [spec.primaryEnv, ...spec.optional]) {
    if (result.env[name] !== undefined) continue;
 ...
 */
export function childEnv(base: Env, overlay: Record<string, string>): Record<string, string | undefined> {
  return { ...base, ...overlay };
}
