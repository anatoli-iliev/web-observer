...
export function readSkillVersion(skillFile: string): string | null {
  let text: string;
  try {
 ...
 */
export function skillSearchDirs(env: Env): string[] {
 ...
 */
function configuredWorkspace(stateDir: string): string | null {
  try {
    const document = JSON.parse(readFileSync(path.join(stateDir, "openclaw.json"), "utf8")) as {
 ...
 */
export function findSkill(slug: string, env: Env, overrideVar: string): SkillLocation | null {
