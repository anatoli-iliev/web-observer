 ...
 */
export const GA4_PRESETS: readonly string[] = [
  "report",
  "compare",
  "live",
  "query",
...
export type Ga4Status =
  ...
  | { kind: "unusable"; location: SkillLocation; message: string }
  | { kind: "ready"; location: SkillLocation; entry: string };
 ...
 */
export function ga4Status(env: Env): Ga4Status {
  const location = findSkill(GA4_SLUG, env, GA4_DIR_VAR);
  if (location === null) {
    return {
      kind: "absent",
      message:
        `the ${GA4_SLUG} skill is not installed, so traffic questions cannot be answered. ` +
  ...
  if (!fileExists(entry)) {
    return {
      ...
      location,
      message:
        `${GA4_SLUG} is installed at ${location.dir} but ${entry} is missing, so it cannot ` +
...
export async function runGa4(
  status: Extract<Ga4Status, { kind: "ready" }>,
...
export function rejectUnknownPreset(argv: readonly string[]): string | null {
  ...
  if (preset.startsWith("-")) {
    return (
      `the first argument must be a preset, not ${preset}. ` +
...
export function digestArgv(config: Config): string[] {
  const argv = [config.ga4.digest.preset, "--since", config.ga4.digest.since];
  if (config.ga4.property !== null) argv.push("--property", config.ga4.property);
  return argv;
}
