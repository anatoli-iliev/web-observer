...
import { ConfigError, emptyConfig, parseConfig, type Config } from "../config.js";
import { cliEntry, configPath, statePath, type Env } from "../paths.js";
...
import { line, note, type Streams } from "./render.js";

/** A configuration read from disk, with enough context for a good message. */
type LoadedConfig = {
  config: Config | null;
  error: string | null;
  exists: boolean;
...
function loadConfig(file: string): LoadedConfig {
  let text: string;
  try {
    text = readFileSync(file, "utf8");
  } catch (error) {
    const code = (error as { code?: string }).code;
    if (code === "ENOENT") return { config: null, error: null, exists: false };
    return {
      config: null,
      error: `${file} could not be read: ${(error as Error).message}`,
      exists: true,
    };
  }
  ...
  try {
    ...
    return {
      ...
      error: `${file} is not valid JSON: ${(error as Error).message}`,
      exists: true,
    };
  }
  try {
    ...
    if (error instanceof ConfigError) return { config: null, error: error.message, exists: true };
    throw error;
  }
}

/** List existing OpenClaw cron job names, or null when they cannot be listed. */
async function listCronJobNames(): Promise<string[] | null> {
  ...
  try {
    ...
    const rows = Array.isArray(document)
      ? document
      : ((document as { jobs?: unknown }).jobs as unknown);
    if (!Array.isArray(rows)) return null;
    return rows
      .map((row) =>
        typeof row === "object" && row !== null ? (row as { name?: unknown }).name : undefined,
      )
 ...
 */
export async function main(argv: readonly string[], env: Env, streams: Streams): Promise<number> {
  let args;
  try {
    args = parseArgs(argv);
  } catch (error) {
  ...
  if (!loaded.exists) {
    note(
      streams,
      `error: no configuration at ${configFile}. Copy examples/config.json from this skill's ` +
  ...
  switch (args.command) {
    case "check":
      return await runCheckCommand(
        { config, configFile, stateFile, streams, deps: liveDeps },
        { json: args.json, strict: args.strict, only: args.only, dryRun: args.dryRun },
      );

    case "watch":
      return await runWatchCommand(
        { config, configFile, stateFile, streams, deps: liveDeps },
        { dryRun: args.dryRun },
      );
    ...
    case "vercel-watch": {
      ...
      const outcome = await runVercelWatch(
        { config, configFile, stateFile, streams, env, now: liveDeps.now },
        {
          status,
          credentials,
    ...
    case "digest": {
      ...
      return await runDigestCommand(
        { config, configFile, streams, env },
        {
          status,
          credentials,
          invoke: async (delegateArgv) => {
    ...
    case "schedule":
      return await runScheduleCommand(
        { config, configFile, streams, cliPath: cliEntry(import.meta.url) },
        { apply: args.apply, json: args.json },
        {
          runOpenclaw: async (openclawArgv) =>
            await runProcess("openclaw", openclawArgv, { timeoutMs: 60_000 }),
