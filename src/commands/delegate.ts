...
import { childEnv, collectCredentials, type CredentialResult } from "../bridge/credentials.js";
import type { RunResult } from "../bridge/delegate.js";
import {
  digestArgv,
  ga4Status,
  GA4_SLUG,
  rejectUnknownPreset as rejectGa4Preset,
  runGa4,
  type Ga4Status,
} from "../bridge/ga4.js";
import {
  prepare as prepareVercel,
  rejectUnknownPreset as rejectVercelPreset,
  runVercel,
  VERCEL_LOG_PRESETS,
...
function relay(streams: Streams, result: RunResult, what: string): number {
  ...
  if (result.stdout !== "") streams.out(result.stdout);
  if (result.stderr !== "") streams.err(result.stderr);
...
export async function runVercelCommand(
  ...
  const preset = passthrough[0] as string;
...
export async function runGa4Command(
  context: DelegateContext,
  passthrough: readonly string[],
): Promise<number> {
  ...
  const result = await runGa4(
    status,
    passthrough,
    childEnv(context.env, credentials.env) as Env,
  );
...
export async function runDigestCommand(
  context: DelegateContext & { configFile: string },
  deps: DigestDeps,
  ...
  if (!context.config.ga4.enabled) {
    note(
      context.streams,
      `error: the ga4 module is switched off in ${context.configFile}. Set ga4.enabled to ` +
