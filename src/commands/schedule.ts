...
export type PlannedJob = {
  ...
  schedule: { kind: "every"; value: string } | { kind: "cron"; value: string };
...
function timeoutFor(worstCaseMs: number, floorSeconds: number): number {
 ...
 */
export function plannedJobs(config: Config): PlannedJob[] {
  ...
  if (config.uptime.enabled && config.uptime.watches.length > 0) {
    ...
    jobs.push({
      ...
      why:
        `ticks every ${plural(config.uptime.tickMinutes, "minute")} and checks whichever ` +
  ...
  if (config.vercel.enabled && config.vercel.errors.enabled) {
    jobs.push({
      ...
      timeoutSeconds: 180,
      why:
        `polls the last ${plural(config.vercel.errors.windowMinutes, "minute")} of error logs ` +
  ...
  if (config.vercel.enabled && config.vercel.budget.enabled) {
    jobs.push({
      ...
      timeoutSeconds: 180,
      why:
        "checks Core Web Vitals against your thresholds every " +
 ...
 */
export function cronAddArgv(job: PlannedJob, config: Config, cliPath: string): string[] {
  ...
  argv.push("--announce");
  if (config.notify.channel !== null) argv.push("--channel", config.notify.channel);
  if (config.notify.to !== null) argv.push("--to", config.notify.to);
  return argv;
}

/** The command as a person would type it, wrapped for readability. */
function printable(argv: readonly string[]): string {
...
export type ScheduleDeps = {
  ...
  runOpenclaw: (argv: readonly string[]) => Promise<{ code: number; stdout: string; stderr: string }>;
  /** Names of cron jobs that already exist. */
  existingJobNames: () => Promise<string[]>;
};

export async function runScheduleCommand(
  context: { config: Config; configFile: string; streams: Streams; cliPath: string },
  ...
  if (jobs.length === 0) {
    note(
      streams,
      `error: no module in ${context.configFile} is enabled, so there is nothing to schedule. ` +
  ...
  if (config.notify.channel === null || config.notify.to === null) {
    ...
    note(
      streams,
      `error: notify.channel and notify.to are not both set in ${context.configFile}, so a ` +
  ...
  if (options.json) {
    ...
    if (!options.apply) return EXIT_OK;
  }
