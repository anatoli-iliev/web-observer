...
export const WATCH_DEFAULTS: WatchDefaults = {
  intervalMinutes: 5,
  method: "GET",
  expectStatus: [200],
  expectBody: null,
  ...
  for (const key of Object.keys(value)) {
    if (!known.includes(key)) {
      ...
      throw new ConfigError(
        `${path}.${key}`,
        `is not a setting Web Observer reads; ${path} accepts ${known.join(", ")}.${hint} ` +
...
function requireObject(path: ConfigPath, value: unknown): Record<string, unknown> {
...
function readBoolean(path: ConfigPath, value: unknown, fallback: boolean): boolean {
...
function readString(path: ConfigPath, value: unknown, fallback: string): string {
  if (value === undefined) return fallback;
...
function readHeaders(path: ConfigPath, value: unknown, fallback: Record<string, string>) {
  if (value === undefined) return fallback;
 ...
 */
function readStatusExpectation(path: ConfigPath, value: unknown): StatusExpectation {
  ...
  if (typeof value === "string") {
    if (!STATUS_CLASS.test(value)) {
      throw new ConfigError(
        path,
        `is ${JSON.stringify(value)}; a string status must be a class such as "2xx" or "5xx". ` +
  ...
  if (Array.isArray(value)) {
    if (value.length !== 2) {
      throw new ConfigError(
        path,
        `has ${value.length} items; a range is exactly two, [min, max], both inclusive`,
      );
    }
    const [min, max] = value;
    for (const bound of [min, max]) {
      if (typeof bound !== "number" || !Number.isInteger(bound) || bound < 100 || bound > 599) {
        throw new ConfigError(
          path,
          "must be a range of two HTTP statuses between 100 and 599, for example [200, 399]",
        );
      }
    }
  ...
  throw new ConfigError(
    path,
    `must be a status code (200), a class ("2xx") or a range ([200, 399]), not ${describe(value)}`,
  );
}

function readStatusExpectations(
  path: ConfigPath,
  value: unknown,
  fallback: StatusExpectation[],
): StatusExpectation[] {
  if (value === undefined) return fallback;
  ...
  if (items.length === 0) {
    throw new ConfigError(
      path,
      "is empty, so no response could ever pass. Name at least one status, class or range",
    );
  }
  return items.map((item, index) =>
    readStatusExpectation(Array.isArray(value) ? `${path}[${index}]` : path, item),
  );
}
 ...
 */
export function readWatchUrl(path: ConfigPath, value: unknown): string {
  if (typeof value !== "string" || value.trim() === "") {
  ...
  try {
    ...
    throw new ConfigError(
      path,
      `is not a URL: ${JSON.stringify(value)}. Include the scheme, as in https://example.com`,
    );
  }
  if (parsed.protocol !== "http:" && parsed.protocol !== "https:") {
    throw new ConfigError(
      path,
      `has scheme ${parsed.protocol.replace(":", "")}; Web Observer checks http and https only`,
    );
  }
  if (parsed.username !== "" || parsed.password !== "") {
    throw new ConfigError(
      path,
      "carries credentials in the URL. Put them in a header instead, so the secret " +
 ...
 */
function readBodyRegex(path: ConfigPath, value: unknown, fallback: string | null): string | null {
...
function readWatchDefaults(path: ConfigPath, value: unknown, base: WatchDefaults): WatchDefaults {
  if (value === undefined) return base;
  const object = requireObject(path, value);
  rejectUnknownKeys(
    path,
    object,
    WATCH_KEYS.filter((key) => key !== "id" && key !== "url"),
  );
  const method = readString(`${path}.method`, object["method"], base.method).toUpperCase();
  if (!SAFE_METHODS.includes(method)) {
    throw new ConfigError(
      `${path}.method`,
      `is ${method}; Web Observer only issues ${SAFE_METHODS.join(", ")}, because a health ` +
  ...
  return {
    ...
    method,
    expectStatus: readStatusExpectations(
      `${path}.expectStatus`,
      object["expectStatus"],
      base.expectStatus,
    ),
    expectBody: readNullableString(`${path}.expectBody`, object["expectBody"], base.expectBody),
    expectBodyRegex: readBodyRegex(
      `${path}.expectBodyRegex`,
      object["expectBodyRegex"],
      base.expectBodyRegex,
    ),
    ...
    recoveryThreshold: readInteger(
      `${path}.recoveryThreshold`,
      object["recoveryThreshold"],
      base.recoveryThreshold,
      1,
      100,
    ),
    followRedirects: readInteger(
...
function readWatch(path: ConfigPath, value: unknown, defaults: WatchDefaults): Watch {
  const object = requireObject(path, value);
  ...
  if (id === "") {
    throw new ConfigError(
      `${path}.id`,
      "is required. It keys this watch's saved state, so renaming it forgets whether " +
  ...
  if (!ID_SHAPE.test(id)) {
    throw new ConfigError(
      `${path}.id`,
      `is ${JSON.stringify(id)}; an id may hold letters, digits, dot, dash and underscore, ` +
  ...
  const overrides: Record<string, unknown> = { ...object };
  delete overrides["id"];
  ...
  if (resolved.expectBody !== null && resolved.expectBodyRegex !== null) {
    throw new ConfigError(
      `${path}.expectBodyRegex`,
      "is set alongside expectBody. Use one: a substring or a pattern, not both, " +
  ...
  if (resolved.method === "HEAD" && (resolved.expectBody !== null || resolved.expectBodyRegex !== null)) {
    throw new ConfigError(
      `${path}.method`,
      "is HEAD, which returns no body, but a body expectation is configured. " +
...
function readUptime(path: ConfigPath, value: unknown): UptimeConfig {
  if (value === undefined) {
  ...
  if (rawWatches !== undefined) {
    ...
    watches = rawWatches.map((watch, index) =>
      readWatch(`${path}.watches[${index}]`, watch, defaults),
    );
  }
  ...
  for (const [index, watch] of watches.entries()) {
    ...
    if (previous !== undefined) {
      throw new ConfigError(
        ...
        `repeats the id ${JSON.stringify(watch.id)} already used by watches[${previous}]. ` +
  ...
  if (enabled && watches.length === 0) {
    throw new ConfigError(
      `${path}.watches`,
      "is empty while uptime is enabled, so there is nothing to check. Add a watch, " +
...
function readMetrics(path: ConfigPath, value: unknown): Record<string, number> {
  ...
  for (const [name, threshold] of Object.entries(object)) {
    if (typeof threshold !== "number" || !Number.isFinite(threshold) || threshold < 0) {
      throw new ConfigError(
        `${path}.${name}`,
        `must be a non-negative number, not ${describe(threshold)}`,
      );
    }
...
function readVercel(path: ConfigPath, value: unknown): VercelConfig {
  const disabled: VercelConfig = {
    enabled: false,
    project: null,
    errors: {
      enabled: false,
  ...
  if (errorsRaw !== undefined) {
    ...
    const intervalMinutes = readInteger(
      `${path}.errors.intervalMinutes`,
      errorsObject["intervalMinutes"],
      15,
      1,
      1_440,
    );
    const windowMinutes = readInteger(
      `${path}.errors.windowMinutes`,
      errorsObject["windowMinutes"],
      Math.max(intervalMinutes + 5, 20),
      1,
      1_440,
    );
    if (windowMinutes <= intervalMinutes) {
      throw new ConfigError(
        `${path}.errors.windowMinutes`,
        `is ${windowMinutes}, which does not exceed intervalMinutes of ${intervalMinutes}. ` +
    ...
    errors = {
      ...
      includeMessages: readBoolean(
        `${path}.errors.includeMessages`,
        errorsObject["includeMessages"],
        false,
      ),
    };
  }
  ...
  if (budgetRaw !== undefined) {
    ...
    const budgetEnabled = readBoolean(
      `${path}.budget.enabled`,
      budgetObject["enabled"],
      Object.keys(metrics).length > 0,
    );
    if (budgetEnabled && Object.keys(metrics).length === 0) {
      throw new ConfigError(
        `${path}.budget.metrics`,
        "is empty while the budget check is enabled, so there is no threshold to test. " +
    ...
    budget = {
      ...
      intervalMinutes: readInteger(
        `${path}.budget.intervalMinutes`,
        budgetObject["intervalMinutes"],
        360,
        1,
        10_080,
      ),
  ...
  if (enabled && project === null) {
    throw new ConfigError(
      `${path}.project`,
      "is required when the Vercel module is enabled: every query names exactly one " +
...
function readGa4(path: ConfigPath, value: unknown): Ga4Config {
  const disabled: Ga4Config = {
    enabled: false,
    property: null,
  ...
  if (digestRaw !== undefined) {
    ...
    if (fields.length !== 5) {
      throw new ConfigError(
        `${path}.digest.cron`,
        `has ${fields.length} fields; a schedule is five, as in "0 9 * * 1" for 09:00 on Mondays`,
      );
    }
    digest = {
...
function readNotify(path: ConfigPath, value: unknown): NotifyConfig {
  if (value === undefined) return { channel: null, to: null };
  const object = requireObject(path, value);
 ...
 */
export function parseConfig(document: unknown): Config {
 ...
 */
export function allowlistOf(watches: readonly Watch[]): ReadonlySet<string> {
 ...
 */
export function allowedUrl(url: string, allowlist: ReadonlySet<string>): URL | null {
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
...
function statusMatchesOne(status: number, expectation: StatusExpectation): boolean {
...
export function statusMatches(status: number, expectations: readonly StatusExpectation[]): boolean {
...
export function describeStatusExpectations(
  expectations: readonly StatusExpectation[],
): string {
  return expectations
    .map((expectation) =>
      Array.isArray(expectation) ? `${expectation[0]}-${expectation[1]}` : String(expectation),
    )
    .join(", ");
}
