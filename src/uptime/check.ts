...
import { allowedUrl, statusMatches, describeStatusExpectations, type Watch } from "../config.js";
...
export function classifyError(error: unknown): { reason: FailureReason; detail: string } {
  ...
  while (current !== null && current !== undefined && !seen.has(current)) {
    ...
    if (current instanceof Error) {
      ...
      if (code === "" && typeof candidate === "string") code = candidate;
...
async function attemptOnce(
  ...
  deps: CheckDeps,
  signal: AbortSignal,
): Promise<{ ok: true; status: number; url: string } | { ok: false; reason: FailureReason; detail: string; status: number | null; url: string }> {
  let url = watch.url;
  for (let hop = 0; ; hop += 1) {
    ...
    // Configured headers go only to the watch's own host. The allowlist is
    // shared by every watch, so without this a redirect could carry one watch's
    // credential to another watch's host.
    const sameHost = target.host.toLowerCase() === new URL(watch.url).host.toLowerCase();

    let response: Response;
    try {
      response = await deps.fetch(target.toString(), {
        method: watch.method,
        headers: sameHost ? watch.headers : {},
        redirect: "manual",
        signal,
      });
    } catch (error) {
    ...
    if (isRedirect(status) && hop < watch.followRedirects) {
      ...
      // The allowlist compares hosts, and https and http share one. A downgrade
      // would send any configured header in the clear, so it is refused here.
      if (target.protocol === "https:" && new URL(next).protocol === "http:") {
        return {
          ok: false,
          reason: "redirect-off-allowlist",
          detail:
            `a redirect downgraded ${safeHost(next)} from https to http, which is never ` +
            "followed. Point the watch at the http URL if that is expected",
          status,
          url: target.toString(),
        };
      }
...
async function evaluate(
  ...
  response: Response,
  url: string,
): Promise<{ ok: true; status: number; url: string } | { ok: false; reason: FailureReason; detail: string; status: number; url: string }> {
  const status = response.status;
  ...
  if (watch.expectBody !== null && !text.includes(watch.expectBody)) {
    return {
      ...
      detail:
        `returned ${status} but the body does not contain ${JSON.stringify(watch.expectBody)}` +
  ...
  if (watch.expectBodyRegex !== null && !new RegExp(watch.expectBodyRegex).test(text)) {
    return {
      ...
      reason: "body-mismatch",
      detail:
        `returned ${status} but the body does not match /${watch.expectBodyRegex}/` +
        ` (${text.length} characters read)${truncationNote}`,
      status,
      url,
    };
...
export const liveDeps: CheckDeps = {
