...
import { REASON_TEXT, type UptimeEvent } from "./decide.js";
...
function eventLines(event: UptimeEvent): string[] {
  if (event.kind === "down") {
    const { watch, result } = event;
    const lines = [`🔴 ${watch.id} is DOWN: ${watch.url}`];
    lines.push(`- Reason: ${REASON_TEXT[result.reason]} (${result.detail})`);
    lines.push(
      `- Confirmed over ${watch.failureThreshold} consecutive check${
        watch.failureThreshold === 1 ? "" : "s"
      }, ${result.attemptsUsed} request${result.attemptsUsed === 1 ? "" : "s"} on the last one`,
    );
 ...
 */
export function formatEvents(events: readonly UptimeEvent[]): string {
  if (events.length === 0) return SILENT_TOKEN;
  ...
  if (events.length === 1) return blocks[0] as string;
